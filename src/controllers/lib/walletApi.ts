...
export default class WalletApiClient {
  ...
  async initFungibleToken(address, token) {
    return this.post(`/v1/accounts/${address}/fungible-tokens/${token}`);
  }

  async getFungibleToken(address, token) {
    return this.get(`/v1/accounts/${address}/fungible-tokens/${token}`);
  }
  ...
  async getFungibleTokenDeposits(from, token) {
    return this.get(`/v1/accounts/${from}/fungible-tokens/${token}/deposits}`);
  }
